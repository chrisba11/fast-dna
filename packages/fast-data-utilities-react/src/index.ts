...
export interface DataLocationFromDataTypeConfig {
    schema: any;
    data: any;
...
/**
 * Finds a subset of locations that are to be handled by a plugin
 */
function getPluginLocationsFromSchema(schema: any, schemaLocations: any): string[] {
    return schemaLocations.filter(
        (schemaLocation: string): boolean => {
            return (
                !!schemaLocation.match(/(reactProperties|properties)\..+?\b/) &&
                typeof get(schema, schemaLocation)[pluginResolverIdKeyword] === "string"
            );
        }
    );
}
...
/**
 * Callback to determine if a string is found within an array of strings
 */
function stringPartialFindIndexCallback(location: string): any {
    return (locationItem: string): boolean => {
        return location.includes(locationItem);
    };
}
